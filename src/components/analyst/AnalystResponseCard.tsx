...
export function AnalystResponseCard({ response }: { response: AnalystResponse }) {
  ...
  const supportingPoints = (response.supportingPoints ?? []).filter(Boolean).slice(0, 3)
  const hasRichDetails = Boolean(
    response.metrics?.length || response.sections?.length || response.findings?.length || response.chart ||
    response.table || response.plan?.length || response.recommendations?.length || response.actions?.length,
  )
  return (
    <article className="rounded-2xl border border-obsidian-900/[0.08] bg-white/80 shadow-soft animate-fade-up">
      <div className="flex items-start gap-3 border-b border-obsidian-900/[0.06] p-5 sm:p-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 whitespace-pre-line text-sm leading-relaxed text-stone-700">{response.answer ?? response.summary ?? response.title}</p>
          {supportingPoints.length > 0 && (
            <ul className="mt-3 space-y-1.5 text-sm leading-relaxed text-stone-600">
              {supportingPoints.map((point, i) => <li key={i} className="flex gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-gold-500" />{point}</li>)}
            </ul>
          )}
          {response.partial && (
      ...
      {(hasRichDetails || response.sources?.length || response.followUp) && (
        <div className="border-t border-obsidian-900/[0.06] px-5 pt-4 sm:px-6">
          {response.followUp && <p className="text-sm italic text-stone-600">{response.followUp}</p>}
          {hasRichDetails && (
            <button type="button" onClick={() => setExpanded((value) => !value)} className="mt-3 text-xs font-semibold text-gold-700 hover:text-gold-800">
              {expanded ? 'Hide deeper analysis' : 'Show deeper analysis'}
            </button>
          )}
        </div>
      )}

      <div className={cn('space-y-5 p-5 sm:p-6', !expanded && 'hidden')}>
        {/* Metrics */}
      ...
      {response.sources && response.sources.length > 0 && <div className="px-5 pb-5 sm:px-6"><SourceList sources={response.sources} /></div>}
    </article>
  )
}
